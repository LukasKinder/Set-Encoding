...
def write_manifest(output_path, target_manifest, ensure_ascii):
...
def run_evaluation_per_task(task_config: dict, predictions_file: str, verbose: int = 0):
    ...
    if verbose != 0:
        print('=' * 40)
        for i, (input_i, reference, predict) in enumerate(zip(inputs, references, predicts)):
            print(f'Input     : {input_i}')
